...
Use when you need more complex or conditional event tracking that is not directly tied to component lifecycle or DOM events. The `useTracker` hook allows you to access context from `Track.Provider` and execute defined tracking logic imperatively.
...
- [`useTracker`](/docs/use-tracker): Learn how to implement custom tracking with the hook.
